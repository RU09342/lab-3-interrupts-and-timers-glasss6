...
void main(void)
{
    ...
    PM5CTL0 &= ~LOCKLPM5;           // Disable the GPIO power-on default high-impedance modePM5CTL0 &= ~LOCKLPM5;           // Disable the GPIO power-on default high-impedance mode
    ...
    P1REN |= (BIT1 + BIT2);         // puller-resistor on the button pin P1.1
    P1OUT |= (BIT1 + BIT2);         // writes "1" to portpin, telling the resistor to pullup
    P1IES |= (BIT1 + BIT2);         // triggers when you press the button, "~BIT3" for when release
    P1IE |= (BIT1 + BIT2);          // enables selector-mask for generating interrupts on the pin
...
__interrupt void Port_1(void) // function called when the port is interrupted (button pressed)
{
    ...
    else if(P1IFG & BIT2)
    {
        ...
        P1IFG &= ~BIT2;   // P1.2 (button) IFG cleared
